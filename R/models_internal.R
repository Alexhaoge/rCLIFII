...
#'
.mod1 <- function(mij, nij, tauij, mtau=1000) {
...
#' @rdname models_internal
#'
.mod2 <- function(mij, nij, tauij, mtau=1000) {
  ind = which(tauij<=mtau)
  mij <- mij[ind]
  nij <- nij[ind]
  tauij <- tauij[ind]
...
#' @rdname models_internal
#'
.mod3 <- function(mij, nij, tauij, mtau=1000) {
  ind = which(tauij<=mtau)
  mij <- mij[ind]
  nij <- nij[ind]
  tauij <- tauij[ind]
...
#'
#'
.mod4 <- function(Aij, Ai, tauij, mtau=1000){
  ind = which(tauij<=mtau)
  Aij <- Aij[ind]
  Ai <- Ai[ind]
  LL <- function(theta){
...
#' @rdname models_internal
#'
.mod5 <- function(Aij, Ai, tauij, mtau=1000) {
  ind = which(tauij<=mtau)
  Aij <- Aij[ind]
  Ai <- Ai[ind]
  tauij <- tauij[ind]
...
#' @rdname models_internal
#'
.mod6 <- function(Aij, Ai, tauij, mtau=1000) {
  ind = which(tauij<=mtau)
  Aij <- Aij[ind]
  Ai <- Ai[ind]
  tauij <- tauij[ind]
...
lir.model.res <- function(model, mij, nij, tauij, mtau){
  if(model=='mod1') {
    est <- .mod1(mij, nij, tauij, mtau)
    sder <- sum(-mij/est$par^2-(nij-mij)/((1-est$par)^2))
    H <- matrix(sder)
    K = 1
  }
  if(model=='mod2') {
    est <- .mod2(mij, nij, tauij, mtau)
    alpha <- est$par[1]
    beta <- est$par[2]
...
lar.model.res <- function(model, Aij, Ai, tauij, mtau){
  if(model=='mod4') {
    est <- .mod4(Aij, Ai, tauij, mtau)
    sder <- sum(-Aij/est$par^2-(Ai-Aij)/((1-est$par)^2))
    H <- matrix(sder)
    K = 1
  }
  if(model=='mod5') {
    est <- .mod5(Aij, Ai, tauij, mtau)
    alpha <- est$par[1]
    beta <- est$par[2]
